...
Before spawning investigators, discover the configured MCP servers per the **harness** skill. When the harness has no MCP discovery or no MCP support, record which evidence categories you cannot query and proceed with the rest. Say so in the coverage map.
...
Subagent config (each), per the **harness** skill:
- model: the `why investigators` role from the pstack model config, adapter default otherwise
- mode: agent, not readonly. **Do not use readonly mode** where the harness strips MCP access there, since that disables MCP-backed investigators entirely. The source control investigator would be safe in readonly, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent, per the **harness** skill:

- model: the `why synthesizer` role from the pstack model config, adapter default otherwise
- mode: agent, not readonly, where the harness strips MCPs in readonly mode. The synthesizer's quality check spot-verifies citations, which can require MCP access. Readonly mode defeats that.
