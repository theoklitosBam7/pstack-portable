...
3. Pick the runners. Use the `arena runners` panel from the pstack model config, per the **harness** skill. Otherwise one runner each on up to four distinct model families you can spawn, the strongest judgment model included. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
4. Assign output paths. Each candidate writes to its own location (a git worktree where possible, otherwise `/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to the same path is shared mutable state and fails the the **separate-before-serializing-shared-state** principle skill test.

## Phase B: Fan out

Spawn all N subagents in one message in background mode, per the **harness** skill, each with the task, the path to the shared grounding, its own output path, and instructions to produce both the artifact and a short rationale.
