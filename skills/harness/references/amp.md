...
`~/.config/pstack/models`. `AGENTS.md` is the always-on surface, cwd up to `$HOME`, plus `~/.config/amp/AGENTS.md`. Reference the config path there for per-session loading.
