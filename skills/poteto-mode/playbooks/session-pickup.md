...
1. Locate the prior trail. A local transcript located with the find-transcripts operation in the **harness** skill, staying inside the active workspace, a remote-agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
