...
3. Explore in subagents with the `poteto-agent` persona, per the **harness** skill, and an explicit model role per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Resolve the forge once per `../references/forge.md`, and use that choice for every PR operation.
